...
장애 로그와 실패 이벤트가 여러 위치에 흩어져 있어 추적이 어렵다.
운영 DB나 운영 클러스터에 직접 접근해야만 원인 분석이 가능한 경우가 많다.
개발자가 복구를 위해 운영망에 직접 접근하면 보안과 안정성 측면에서 큰 리스크가 발생한다.
수정된 코드가 실제로 문제를 해결했는지, 또 정상 동작까지 보장하는지 검증하기 어렵다.
