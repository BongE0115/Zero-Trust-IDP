...
브라우저에서 localhost:8081 접속 ➔ [실패 요청 보내기] 클릭 (터미널 4 ➔ 터미널 1)

터미널 2가 에러를 뿜으며 orders-dlq 로 메시지 던짐 (터미널 2 ➔ 터미널 1)

터미널 3이 그걸 주워서 Slack 채널로 알람과 버튼을 전송 (터미널 1 ➔ 터미널 3 ➔ Slack)

사용자가 Slack에서 [Create Sandbox] 버튼 클릭

Slack 서버가 터미널 5(Ngrok) 를 거쳐 터미널 4로 신호를 보냄 (Slack ➔ 터미널 5 ➔ 터미널 4)

터미널 4가 신호를 해석하고 GitHub API를 호출하여 Actions 실행! (터미널 4 ➔ GitHub)
